...
module Log = struct
  ...
  let () = at_exit (fun () -> close_out_noerr log_file)
  ...
  let write identifier message =
    Printf.ksprintf (output_string log_file) "%s [%07.3f]: %s\n" identifier
      (mod_float (elapsed ()) 1000.)
      message;
    flush log_file

  let log k = k (fun identifier -> Printf.ksprintf (write identifier))
end

let log = Log.log

let run_test : test -> outcome Lwt.t =
 fun test ->
  if test.skip_if_this_is_false () = false then (
    ...
    (Lwt.async_exception_hook :=
       fun exn -> Lwt.wakeup_later async_exception_occurred (Exception exn));
      ...
      (fun () ->
        let test_completion_promise =
          Lwt.try_bind
            (fun () -> test.run ())
            (fun test_did_pass ->
              if test_did_pass then Lwt.return Passed else Lwt.return Failed)
            (function
              | Skip -> Lwt.return Skipped
              | exn_raised_by_test -> Lwt.return (Exception exn_raised_by_test))
        in
...
let contains_dup_tests suite tests =
  let names =
    List.map (fun t -> "suite:" ^ suite ^ " test:" ^ t.test_name) tests
  in
  ...
  in
  if List.length dups > 0 then Some dups else None
...
let run_test_suite : suite -> (string * outcome) list Lwt.t =
 fun suite ->
...
let show_failures : (string * outcome) list -> unit =
  List.iter (fun (test_name, outcome) ->
      ...
      | Exception exn ->
          Printf.eprintf "Test '%s' raised '%s'\n" test_name
            (Printexc.to_string exn))

type ('a, 'b) aggregated_outcomes = ('a * ('b * outcome) list) list

let fold_over_outcomes :
    ('a -> outcome -> 'a) -> 'a -> (_, _) aggregated_outcomes -> 'a =
 fun f init outcomes ->
  List.fold_left
    (fun accumulator (_suite_name, test_outcomes) ->
      List.fold_left
        (fun accumulator (_test_name, test_outcome) ->
          f accumulator test_outcome)
        accumulator test_outcomes)
    init outcomes

let count_ran : (_, _) aggregated_outcomes -> int =
 fun outcomes ->
  outcomes
  |> fold_over_outcomes
       (fun count -> function Skipped -> count | _ -> count + 1)
       0

let count_skipped : (_, _) aggregated_outcomes -> int =
 fun outcomes ->
  outcomes
  |> fold_over_outcomes
       (fun count -> function Skipped -> count + 1 | _ -> count)
       0
...
let run library_name suites =
  ...
  let rec loop_over_suites aggregated_outcomes suites =
    ...
    | [] ->
        ...
        Lwt.return_unit
    | suite :: rest ->
        Lwt.bind (run_test_suite suite) (fun outcomes ->
            if not (outcomes_all_ok outcomes) then (
              print_newline ();
              flush stdout;
  ...
  loop_over_suites [] suites |> Lwt_main.run
...
let concurrent library_name suites =
  ...
  let run_test (suite, test) =
    (if suite.skip_suite_if_this_is_false () = false then Lwt.return Skipped
     else run_test test)
    >|= fun outcome ->
  ...
  suites
  |> List.map (fun suite ->
         suite.suite_tests |> List.map (fun test -> (suite, test)))
  |> List.flatten
  ...
  Lwt_list.map_s run_test sequential >>= fun sequential_outcomes ->
  (* Run the tests that can be run concurrently. *)
  concurrent |> Lwt_list.map_p run_test
